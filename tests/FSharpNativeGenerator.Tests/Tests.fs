...
let private projectOptionsWithReferences
...
type AdditionalEchoGenerator() =
    interface IFSharpIncrementalGenerator with
        member _.Initialize context =
            let modules =
                context.AdditionalTextsProvider |> FSharpIncrementalValuesProvider.map _.Text
            ...
            context.RegisterSourceOutput(
                ...
                Action<FSharpSourceProductionContext, FSharpGeneratorProjectSnapshot>(fun productionContext _ ->
                    productionContext.ReportDiagnostic(
                        FSharpGeneratorDiagnostic.create "TEST0001" "reported" FSharpDiagnosticSeverity.Warning
                    ))
            )
...
let Loader_LoadsPublicAttributedGenerator () =
    ...
    Assert.Contains(
        result.Generators,
        fun generator -> generator.TypeName.EndsWith("CliHarnessGenerator", StringComparison.Ordinal)
    )
...
let Loader_RejectsMissingAttribute () =
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            ...
            && diagnostic.Message.Contains("MissingAttributeGenerator", StringComparison.Ordinal)
    )
...
let Loader_RejectsUnsupportedApiVersion () =
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            ...
            && diagnostic.Message.Contains("UnsupportedApiGenerator", StringComparison.Ordinal)
    )

[<Fact>]
let Loader_ReportsAssemblyLoadFailure () =
    let result =
        FSharpGeneratorAssemblyLoader.loadFromPath (Path.Combine(tempRoot (), "missing.dll"))
...
let Adapter_RunsPostInitializationOutput () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(PostInitGenerator(), "Tests/PostInit") :> IFSharpSourceGenerator

    let output =
        adapter.Generate(context root [ Path.Combine(root, "User.fs") ] Map.empty [])
...
let Adapter_RunsSourceOutputForAdditionalFile () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(AdditionalEchoGenerator(), "Tests/Additional") :> IFSharpSourceGenerator

    let output =
        adapter.Generate(
            context root [ Path.Combine(root, "User.fs") ] (Map.ofList [ "schema.txt", "SchemaModule" ]) []
        )
...
let Adapter_MapsDiagnosticsToForkDiagnostics () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(DiagnosticGenerator(), "Tests/Diagnostics") :> IFSharpSourceGenerator
...
let Adapter_ProducesStableGeneratedFileName () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(PostInitGenerator(), "Assembly/Namespace.Type") :> IFSharpSourceGenerator
    ...
    let expected =
        Path.Combine(root, "obj", "Generated", "FSharp", "Assembly", "Namespace.Type", "Prelude.fs")
        |> Path.GetFullPath
...
let Placement_PreludeBeforeFirstOriginal () =
    ...
    let result =
        FSharpGeneratedSourcePlacementResolver.resolve [ "A.fs"; "B.fs" ] [] [ source ]
...
let Placement_BeforeLastSourceFileBeforeLastImplementation () =
    ...
    let result =
        FSharpGeneratedSourcePlacementResolver.resolve [ "A.fsi"; "A.fs"; "Program.fs" ] [] [ source ]
...
let Placement_EndOfProjectRejectedForApplication () =
    ...
    let diagnostics =
        expectError (FSharpGeneratedSourcePlacementResolver.resolve [ "Program.fs" ] [ "--target"; "exe" ] [ source ])
...
let Placement_DuplicateHintRejected () =
    ...
    let diagnostics =
        expectError (FSharpGeneratedSourcePlacementResolver.resolve [ "A.fs" ] [] [ source "Dup"; source "Dup" ])
...
let Placement_MissingAnchorRejected () =
    ...
    let diagnostics =
        expectError (FSharpGeneratedSourcePlacementResolver.resolve [ "A.fs" ] [] [ source ])
...
let Placement_EmptyOriginalsDoesNotThrow () =
    ...
    let generated, _ =
        expectOk (FSharpGeneratedSourcePlacementResolver.resolve [] [] [ source ])
...
let Host_LoadFromConfigurationLoadsGenerators () =
    ...
    let generators, diagnostics =
        host.LoadFromConfiguration
            { FSharpSourceGeneratorConfiguration.empty with
                GeneratorPaths = [ path ] }
...
let Host_RunGeneratorsUpdatesProjectSourceFiles () =
    ...
    let updatedOptions, result =
        host.RunGenerators(
            options,
            [ loaded (GeneratedSymbolGenerator()) "Tests/GeneratedSymbol" ],
            generatorOptions root []
        )
...
let Host_AdditionalFileChangeInvalidatesForkRunCache () =
    ...
    let _, first =
        host.RunGenerators(options, [ generator ], generatorOptions root [ additional ])
        |> Async.RunSynchronously
...
let Adapter_InitializesOncePerAdapterInstance () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(InitializeCountingGenerator(counter), "Tests/InitializeOnce")
...
let Placement_UserImplementationSignatureRejected () =
    ...
    let diagnostics =
        expectError (FSharpGeneratedSourcePlacementResolver.resolve [ "Domain.fs" ] [] [ source ])
...
type ThrowingSourceOutputGenerator() =
    interface IFSharpIncrementalGenerator with
        member _.Initialize context =
            context.RegisterSourceOutput(
                context.ProjectOptionsProvider,
                Action<FSharpSourceProductionContext, FSharpGeneratorProjectSnapshot>(fun _ _ ->
                    invalidOp "source-output boom")
            )
        ...
        member _.Initialize context =
            context.RegisterSourceOutput(
                ...
                Action<FSharpSourceProductionContext, FSharpAnalyzerConfigOptions>(fun productionContext options ->
                    let hasValue =
                        options.GlobalOptions.ContainsKey "build_property.GeneratedModuleName"

                    let source =
                        if hasValue then
                            "module AnalyzerConfigWasSet"
                        else
                            "module AnalyzerConfigWasEmpty"
...
type KindCollectingGenerator() =
    interface IFSharpIncrementalGenerator with
        member _.Initialize context =
            ...
            context.RegisterSourceOutput(
                combined,
                Action<FSharpSourceProductionContext, string option * FSharpAdditionalFileInput list>
                    (fun productionContext (_, files) ->
                        productionContext.AddImplementationSource(
                            "KindCollection",
                            sprintf "module KindCollection\nlet count = %d" files.Length,
                            Prelude
                        ))
            )
        ...
        member _.Initialize context =
            ...
            context.RegisterSourceOutput(
                ...
                Action<FSharpSourceProductionContext, FSharpAdditionalFileInput list>(fun productionContext files ->
                    productionContext.AddImplementationSource(
                        "CompositeExtensionCollection",
                        sprintf "module CompositeExtensionCollection\nlet count = %d" files.Length,
                        Prelude
                    ))
            )
...
let Loader_ConstructorFailureDoesNotHideOtherValidGenerators () =
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            diagnostic.Id = "FSG0002"
            && diagnostic.Message.Contains("ConstructorThrowsGenerator", StringComparison.Ordinal)
    )
...
let Loader_RejectsMissingPublicParameterlessConstructor () =
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            diagnostic.Id = "FSG0002"
            && diagnostic.Message.Contains("NoPublicParameterlessConstructorGenerator", StringComparison.Ordinal)
    )
...
let Adapter_InitializesOnceUnderConcurrentGenerateCalls () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(InitializeCountingGenerator(counter), "Tests/ConcurrentInitialize")
        :> IFSharpSourceGenerator
...
let Adapter_InitializationExceptionBecomesDiagnostic () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(ThrowingInitializationGenerator(), "Tests/ThrowInit") :> IFSharpSourceGenerator
    ...
    Assert.Contains(
        output.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSG0003"
            && diagnostic.Message.Contains("initialize boom", StringComparison.Ordinal)
    )
...
let Adapter_PostInitializationExceptionBecomesDiagnostic () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(ThrowingPostInitializationGenerator(), "Tests/ThrowPostInit")
        :> IFSharpSourceGenerator
    ...
    Assert.Contains(
        output.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSG0004"
            && diagnostic.Message.Contains("post-init boom", StringComparison.Ordinal)
    )
...
let Adapter_SourceOutputExceptionBecomesDiagnostic () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(ThrowingSourceOutputGenerator(), "Tests/ThrowSource") :> IFSharpSourceGenerator
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            ...
            && diagnostic.Message.Contains("source-output boom", StringComparison.Ordinal)
    )
...
let Adapter_AnalyzerConfigProviderIsDeterministicallyEmptyUntilForkContextCarriesAnalyzerConfigs () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(AnalyzerConfigObservingGenerator(), "Tests/AnalyzerConfig")
        :> IFSharpSourceGenerator
...
let Host_AnalyzerConfigProviderReadsGlobalOptions () =
    ...
    let result =
        host.RunGenerators(
            options,
            [ loaded (AnalyzerConfigObservingGenerator()) "Tests/AnalyzerConfigHost" ],
            generatorOptionsWithAnalyzerConfigs root [] [ analyzer ]
        )
        |> Async.RunSynchronously
        |> snd
...
let Host_AdditionalFilesProviderSupportsKindFilterCollectToValueAndMap2 () =
    ...
    let result =
        host.RunGenerators(
            options,
            [ loaded (KindCollectingGenerator()) "Tests/KindCollect" ],
            generatorOptionsWithAnalyzerConfigs root [ jsonFile; sqlFile ] [ analyzer ]
        )
        |> Async.RunSynchronously
        |> snd
...
let Host_AdditionalFilesProviderSupportsCompositeExtensionFilter () =
    ...
    let result =
        host.RunGenerators(
            options,
            [ loaded (CompositeExtensionCollectingGenerator()) "Tests/CompositeExtension" ],
            generatorOptions root [ openApiFile; jsonFile ]
        )
        |> Async.RunSynchronously
        |> snd
...
let AdditionalFileInput_MetadataHelpersExposeCommonValues () =
    ...
    let options =
        System.Collections.Generic.Dictionary<string, string>(dict [ "custom_option", "configured" ])
...
let Placement_RelativeAnchorMatchesAbsoluteOriginalPath () =
    ...
    let generated, diagnostics =
        expectOk (FSharpGeneratedSourcePlacementResolver.resolve [ original ] [] [ source ])
...
let private runScenarioGenerator root userSource additionalPath additionalText analyzerText generator =
    ...
    host.RunGenerators(
        options,
        [ loaded generator "Tests/Scenario" ],
        generatorOptionsWithAnalyzerConfigs root [ additionalPath ] [ analyzer ]
    )
    |> Async.RunSynchronously
    |> snd
...
let private compileScenario root userSource additionalPath additionalText analyzerText generator references =
    ...
    host.Compile(
        argv,
        [ loaded generator "Tests/ScenarioCompile" ],
        generatorOptionsWithAnalyzerConfigs root [ additionalPath ] [ analyzer ]
    )
    |> Async.RunSynchronously
...
let audit: bool = parsed.Features.Audit"""
    ...
    let results, runResult =
        host.ParseAndCheck(
            options,
            [ loaded (JsonConfigGenerator()) "Tests/JsonScenario" ],
            generatorOptionsWithAnalyzerConfigs root [ additional ] [ analyzer ]
        )
        |> Async.RunSynchronously
    ...
    Assert.Contains(
        ...
        fun source -> source.SourceText.Contains("type AppSettings", StringComparison.Ordinal)
    )
...
let Scenario_InvalidJsonReportsDiagnosticWithAdditionalFilePath () =
    ...
    Assert.Contains(
        result.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSGJSON0001"
            && diagnostic.Message.Contains(additional, StringComparison.Ordinal)
    )
...
let loadedPet: Async<MyApp.Api.Pet> = client.GetPetById 1L"""
    ...
    let results, runResult =
        host.ParseAndCheck(
            options,
            [ loaded (OpenApiClientGenerator()) "Tests/OpenApiScenario" ],
            generatorOptionsWithAnalyzerConfigs root [ additional ] [ analyzer ]
        )
        |> Async.RunSynchronously
    ...
    Assert.Contains(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("type PetStoreClient", StringComparison.Ordinal)
    )

    Assert.Contains(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("JsonSerializer.Deserialize", StringComparison.Ordinal)
    )

    Assert.DoesNotContain(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("Unchecked.defaultof", StringComparison.Ordinal)
    )
...
let Scenario_OpenApiUnsupportedResponseShapeReportsDiagnostic () =
    ...
    Assert.Contains(
        result.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSGOPENAPI0004"
            && diagnostic.Message.Contains("response schema reference", StringComparison.Ordinal)
    )
...
let Scenario_CHeaderGeneratesNativeExterns () =
    ...
    let results, result =
        host.ParseAndCheck(
            options,
            [ loaded (NativeHeaderBindingGenerator()) "Tests/NativeScenario" ],
            generatorOptionsWithAnalyzerConfigs root [ additional ] [ analyzer ]
        )
        |> Async.RunSynchronously
...
let Scenario_SqlFileGeneratesTypedQueryVisibleToUserCode () =
    ...
    writeFile
        userFile
        """module User
let row: MyApp.Data.GetUserById.Row = { Id = 1; Name = "Ada"; Email = None }
let parameters: MyApp.Data.GetUserById.Parameters = { Id = 1 }
let executor =
    { new MyApp.Data.SqlExecutor with
        member _.QuerySingle(_commandText, _parameters, map) =
            async { return Some(map (Map.ofList [ "id", box 1; "name", box "Ada"; "email", box (Some "a@example.com") ])) } }
let queryResult: Async<MyApp.Data.GetUserById.Row option> = MyApp.Data.GetUserById.execute executor parameters"""
...
build_metadata.AdditionalFiles.SqlParameterTypes = id:int" ])
    ...
    let results, runResult =
        host.ParseAndCheck(
            options,
            [ loaded (SqlQueryGenerator()) "Tests/SqlScenario" ],
            generatorOptionsWithAnalyzerConfigs root [ additional ] [ analyzer ]
        )
        |> Async.RunSynchronously
    ...
    Assert.Contains(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("module GetUserById", StringComparison.Ordinal)
    )

    Assert.Contains(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("type SqlExecutor", StringComparison.Ordinal)
    )
    ...
    Assert.Contains(
        ...
        fun source ->
            source.SourceText.Contains(
                "let execute (database: SqlExecutor) (parameters: Parameters)",
    ...
    Assert.DoesNotContain(
        runResult.GeneratedSources,
        fun source -> source.SourceText.Contains("return None", StringComparison.Ordinal)
    )
...
let Scenario_StableHintNamesDoNotCollideForSameFileNameInDifferentDirectories () =
    ...
    let result =
        host.RunGenerators(
            options,
            [ loaded (JsonConfigGenerator()) "Tests/JsonStableHints" ],
            generatorOptionsWithAnalyzerConfigs root [ first; second ] [ analyzer ]
        )
        |> Async.RunSynchronously
        |> snd
...
let Scenario_AnalyzerConfigContentChangeInvalidatesForkRunCache () =
    ...
    Assert.Contains(
        ...
        fun source -> source.SourceText.Contains("namespace First.Config", StringComparison.Ordinal)
    )

    Assert.Contains(
        second.GeneratedSources,
        fun source -> source.SourceText.Contains("namespace Second.Config", StringComparison.Ordinal)
    )
...
let Scenario_CHeaderUnsupportedArgumentReportsDiagnosticInsteadOfDroppingArgument () =
    ...
    Assert.Contains(
        ...
        fun diagnostic ->
            diagnostic.Id = "FSGNATIVE0001"
            && diagnostic.Message.Contains("unsigned", StringComparison.Ordinal)
    )
...
let Scenario_SqlMissingParameterTypeReportsDiagnostic () =
    ...
    Assert.Contains(
        result.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSGSQL0003"
            && diagnostic.Message.Contains("@id", StringComparison.Ordinal)
    )
...
let Scenario_SqlUnsupportedMetadataTypeReportsDiagnostic () =
    ...
    Assert.Contains(
        result.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSGSQL0004"
            && diagnostic.Message.Contains("System.Guid", StringComparison.Ordinal)
    )

[<Fact>]
let Scenario_SqlUnsupportedResultCardinalityReportsDiagnostic () =
    let root = tempRoot ()
    let additional = Path.Combine(root, "ListUsers.sql")
    ...
    Assert.Contains(
        result.Diagnostics,
        fun diagnostic ->
            diagnostic.Id = "FSGSQL0005"
            && diagnostic.Message.Contains("many", StringComparison.Ordinal)
    )
...
let executor =
    { new MyApp.Data.SqlExecutor with
        member _.QuerySingle(_commandText, _parameters, map) =
            async { return Some(map (Map.ofList [ "id", box 1; "name", box "Ada" ])) } }
let queryResult: Async<MyApp.Data.GetUserById.Row option> = MyApp.Data.GetUserById.execute executor { Id = 1 }"""
                ...
                Action<FSharpSourceProductionContext, FSharpGeneratorProjectSnapshot>(fun productionContext _ ->
                    productionContext.ReportDiagnostic(
                        { FSharpGeneratorDiagnostic.create
                              ...
                              FSharpDiagnosticSeverity.Warning with
                            FilePath = Some path }
                    ))
            )
...
let Adapter_FilePathDiagnosticFallbackPrefixesMessage () =
    ...
    let adapter =
        IncrementalGeneratorAdapter(FilePathDiagnosticGenerator(diagnosticPath), "Tests/FilePathDiagnostic")
        :> IFSharpSourceGenerator
