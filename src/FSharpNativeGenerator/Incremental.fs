...
type FSharpPostInitializationContext internal (generatorName: string, pending: ResizeArray<PendingGeneratedSource>, diagnostics: ResizeArray<FSharpGeneratorDiagnostic>, cancellationToken: CancellationToken) =
    ...
    member _.ReportDiagnostic(diagnostic: FSharpGeneratorDiagnostic) =
        diagnostics.Add diagnostic

type FSharpSourceProductionContext internal (generatorName: string, pending: ResizeArray<PendingGeneratedSource>, diagnostics: ResizeArray<FSharpGeneratorDiagnostic>, cancellationToken: CancellationToken) =
    member _.CancellationToken = cancellationToken

    member _.AddImplementationSource(hintName: string, sourceText: FSharpSourceText, placement: FSharpGeneratedSourcePlacement) =
        pending.Add
            {
                GeneratorName = generatorName
                HintName = hintName
    ...
    member _.AddSignatureSource(hintName: string, sourceText: FSharpSourceText, companionImplementationHintName: string, placement: FSharpGeneratedSourcePlacement) =
        pending.Add
            {
                GeneratorName = generatorName
                HintName = hintName
...
type internal RegisteredSourceOutput =
    FSharpGeneratorProjectSnapshot -> FSharpSourceProductionContext -> unit

type FSharpIncrementalGeneratorInitializationContext internal (projectOptionsProvider, sourceFilesProvider, additionalTextsProvider, analyzerConfigOptionsProvider, postInitOutputs: ResizeArray<Action<FSharpPostInitializationContext>>, sourceOutputs: ResizeArray<RegisteredSourceOutput>) =
    member _.ProjectOptionsProvider: FSharpIncrementalValueProvider<FSharpProjectOptions> = projectOptionsProvider
    member _.SourceFilesProvider: FSharpIncrementalValuesProvider<FSharpSourceFileSnapshot> = sourceFilesProvider
    member _.AdditionalTextsProvider: FSharpIncrementalValuesProvider<FSharpAdditionalText> = additionalTextsProvider
    member _.AnalyzerConfigOptionsProvider: FSharpIncrementalValueProvider<FSharpAnalyzerConfigOptions> = analyzerConfigOptionsProvider

    member _.RegisterPostInitializationOutput(action: Action<FSharpPostInitializationContext>) =
        postInitOutputs.Add action

    member _.RegisterSourceOutput<'T>(source: FSharpIncrementalValueProvider<'T>, action: Action<FSharpSourceProductionContext, 'T>) =
        sourceOutputs.Add(fun snapshot context -> action.Invoke(context, source.Evaluate snapshot))

    member _.RegisterSourceOutput<'T>(source: FSharpIncrementalValuesProvider<'T>, action: Action<FSharpSourceProductionContext, 'T>) =
