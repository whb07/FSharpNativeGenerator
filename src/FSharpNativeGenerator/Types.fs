...
module internal Hashing =
    let sha256Bytes (text: string) =
        ...
        text |> Encoding.UTF8.GetBytes |> sha.ComputeHash |> ImmutableArray.CreateRange

    let sha256Many (parts: string seq) =
        parts |> String.concat "\u001f" |> sha256Bytes

    let toHex (bytes: ImmutableArray<byte>) =
        bytes |> Seq.toArray |> Convert.ToHexString

module FSharpSourceText =
    let checksum (sourceText: FSharpSourceText) = Hashing.sha256Bytes sourceText.Text
...
module FSharpSourceFileSnapshot =
    ...
    let loadProjectSourceFiles
        (projectOptions: FSharpProjectOptions)
        (tryGetGeneratedText: string -> FSharpSourceText option)
            ...
            | None ->
                diagnostics.Add
                    { FSharpGeneratorDiagnostic.create
                          "FSG0011"
                          (sprintf
                              "Source file '%s' could not be resolved from disk or generated source store."
                              sourceFile)
                          Error with
...
type FSharpMSBuildSourceGeneratorItem = { Include: string }
